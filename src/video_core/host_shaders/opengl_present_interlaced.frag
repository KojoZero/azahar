...
void main() {
    ...
    if (int(screen_row) % 2 == reverse_interlaced) {
        color = linear_filter != 0 ? GammaCorrectSample(color_texture, frag_tex_coord)
                                   : texture(color_texture, frag_tex_coord);
    } else {
        color = linear_filter != 0 ? GammaCorrectSample(color_texture_r, frag_tex_coord)
                                   : texture(color_texture_r, frag_tex_coord);
    }
}
